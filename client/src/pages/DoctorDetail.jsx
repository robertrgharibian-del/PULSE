    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-5 py-8">
      ...
      <div className="rounded-2xl p-4 sm:p-5 mb-6" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
        ...
        <div className="text-xs" style={{ color: "#6B7280" }}>
          Конференция: <b style={{ color: "#374151" }}>{data.doctor.event_name || "—"}</b> ({data.doctor.event_city || "—"})
          {data.doctor.trip_start && <> · {String(data.doctor.trip_start).slice(0, 10)} — {String(data.doctor.trip_end || "").slice(0, 10)}</>}
        </div>
        <div className="mt-3 font-mono text-lg font-bold" style={{ color: "#ED3237" }}>Общий вклад: ${Math.round(data.total_usd).toLocaleString()}</div>
      </div>

      {chartData.length > 0 && (
        <div className="rounded-2xl p-4 mb-6" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
          <div className="text-xs uppercase mb-2" style={{ color: "#6B7280" }}>Вклад по месяцам, $</div>
      ...
      <div className="rounded-2xl p-4 sm:p-5 mb-6" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
        ...
        <div className="grid sm:grid-cols-2 gap-2 text-sm">
          ...
          {data.pharmacies.length === 0 && <div style={{ color: "#6B7280" }}>Аптеки не заданы</div>}
        </div>
      </div>
      ...
      {canEdit && (
        <div className="rounded-2xl p-4 sm:p-5 mb-6" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
          ...
          <div className="grid sm:grid-cols-4 gap-2 text-sm mb-3">
            ...
            <input type="number" placeholder="Кол-во уп." value={qty} onChange={(e) => setQty(e.target.value)} className="bg-transparent border rounded px-2 py-2 font-mono" style={{ borderColor: "#D3D8E4" }} />
          </div>
          {error && <div className="text-sm mb-2" style={{ color: "#DC2626" }}>{error}</div>}
          <button onClick={addLog} disabled={busy} className="px-4 py-2 rounded font-semibold" style={{ background: "#16A34A", color: "#FFFFFF" }}>Добавить</button>
        </div>
      )}
          ...
          {data.log.map((l) => (
            <div key={l.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg px-3 py-2 text-sm" style={{ background: "#EEF1F8" }}>
              ...
              <div className="flex items-center gap-2">
                ...
                {canEdit && <button onClick={() => removeLog(l.id)} className="text-xs" style={{ color: "#DC2626" }}>Удалить</button>}
              </div>
            </div>
          ))}
          {data.log.length === 0 && <div className="text-sm" style={{ color: "#6B7280" }}>Записей пока нет</div>}
        </div>
      </div>
    </div>
  );
