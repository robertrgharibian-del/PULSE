...
function AddDoctorForm({ onCreated }) {
  ...
  if (!open) {
    return <button onClick={() => setOpen(true)} className="px-5 py-2.5 rounded font-semibold mb-6" style={{ background: "#ED3237", color: "#FFFFFF" }}>+ Добавить врача</button>;
  }
  ...
  return (
    <div className="rounded-2xl p-4 sm:p-5 mb-6" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
      ...
      <div className="grid sm:grid-cols-2 gap-3 text-sm mb-4">
        <input placeholder="ФИО врача" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="bg-transparent border rounded px-3 py-2" style={{ borderColor: "#D3D8E4" }} />
        ...
        <input placeholder="Город" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className="bg-transparent border rounded px-3 py-2" style={{ borderColor: "#D3D8E4" }} />
        <input placeholder="Клиника" value={form.clinic} onChange={(e) => setForm({ ...form, clinic: e.target.value })} className="bg-transparent border rounded px-3 py-2" style={{ borderColor: "#D3D8E4" }} />
        <input placeholder="Контакты" value={form.contact} onChange={(e) => setForm({ ...form, contact: e.target.value })} className="bg-transparent border rounded px-3 py-2 sm:col-span-2" style={{ borderColor: "#D3D8E4" }} />
        <input placeholder="Мероприятие (конференция)" value={form.event_name} onChange={(e) => setForm({ ...form, event_name: e.target.value })} className="bg-transparent border rounded px-3 py-2" style={{ borderColor: "#D3D8E4" }} />
        <input placeholder="Город мероприятия" value={form.event_city} onChange={(e) => setForm({ ...form, event_city: e.target.value })} className="bg-transparent border rounded px-3 py-2" style={{ borderColor: "#D3D8E4" }} />
        <label className="text-xs" style={{ color: "#6B7280" }}>Начало поездки
        ...
        <label className="text-xs" style={{ color: "#6B7280" }}>Конец поездки
      ...
      <div className="text-xs uppercase mb-2" style={{ color: "#6B7280" }}>10 индикаторных аптек</div>
      ...
      <div className="flex gap-3">
        <button onClick={submit} disabled={busy} className="px-5 py-2.5 rounded font-semibold" style={{ background: "#16A34A", color: "#FFFFFF" }}>{busy ? "Сохранение…" : "Сохранить врача"}</button>
        <button onClick={() => setOpen(false)} className="px-4 py-2.5 rounded" style={{ background: "#E4E7F0" }}>Отмена</button>
      </div>
    </div>
  );
}
...
export default function DocTrackingList({ user }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-5 py-8">
      ...
      <div className="text-sm mb-6" style={{ color: "#6B7280" }}>Мониторинг выписки препаратов врачами после конференций — по 10 индикаторным аптекам</div>
      ...
      {loading ? <div style={{ color: "#6B7280" }}>Загрузка…</div> : (
        <div className="space-y-2">
          ...
          {doctors.length === 0 && <div className="text-sm" style={{ color: "#6B7280" }}>Пока нет врачей на отслеживании</div>}
        </div>
      )}
    </div>
  );
