...
export default function TeamPage({ user }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-5 py-8">
      ...
      <div className="text-sm mb-6" style={{ color: "#6B7280" }}>Карточки сотрудников, история, план развития</div>
      ...
      {byGroup.map(({ group, mps: groupMps }) => groupMps.length > 0 && (
        <div key={group.id} className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
            {groupMps.map((mp) => (
              <button key={mp.id} onClick={() => setSelectedMp(mp)} className="text-left rounded-2xl p-4" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
      ...
      {ungrouped.length > 0 && (
        <div className="mb-6">
          <div className="text-sm uppercase tracking-wide mb-2" style={{ color: "#6B7280" }}>Без группы</div>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
            {ungrouped.map((mp) => (
              <button key={mp.id} onClick={() => setSelectedMp(mp)} className="text-left rounded-2xl p-4" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
                <div className="font-semibold">{mp.full_name}</div>
      ...
      {mps.length === 0 && <div className="text-sm" style={{ color: "#6B7280" }}>В команде пока нет медпредов</div>}
    </div>
  );
}
