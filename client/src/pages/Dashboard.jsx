...
export default function Dashboard({ role }) {
  ...
  if (!data) return <div className="max-w-6xl mx-auto px-5 py-10" style={{ color: "#6B7280" }}>Загрузка…</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-5 py-8">
      ...
      <div className="text-sm mb-6" style={{ color: "#6B7280" }}>Последние одобренные отчёты по каждому МП</div>
      ...
      <div className="rounded-2xl p-5 mb-6 flex flex-wrap items-center gap-6" style={{ background: "linear-gradient(90deg,#EEF1F8,#F7F8FC)", border: "1px solid #E4E7F0" }}>
        ...
        <div className="flex-1 min-w-[220px] grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div>
            <div className="text-xs uppercase" style={{ color: "#6B7280" }}>План / Факт</div>
            <div className="font-mono text-lg">${data.company.actual_usd.toLocaleString()} / ${data.company.target_usd.toLocaleString()}</div>
          </div>
          <div>
            <div className="text-xs uppercase" style={{ color: "#6B7280" }}>Бонус (сумма по МП)</div>
            <div className="font-mono text-lg" style={{ color: "#ED3237" }}>{data.company.bonus_uzs.toLocaleString()} UZS</div>
          </div>
          <div>
            <div className="text-xs uppercase" style={{ color: "#6B7280" }}>{role === "master" ? "Регионов" : "Территорий"}</div>
          ...
          <div key={rm.id} className="rounded-2xl overflow-hidden" style={{ border: "1px solid #E4E7F0" }}>
            <button onClick={() => setExpanded((e) => ({ ...e, [rm.id]: !e[rm.id] }))}
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: "#6B7280" }}>{rm.mps.length} МП</div>
              </div>
            ...
            {expanded[rm.id] && (
              <div className="divide-y" style={{ borderColor: "#E4E7F0" }}>
                {rm.mps.map((mp) => (
                  <div key={mp.id} className="flex flex-wrap items-center justify-between gap-3 p-3 px-4" style={{ background: "#FFFFFF" }}>
                    <div>
                      ...
                      <div className="text-xs" style={{ color: "#6B7280" }}>{mp.territory || "—"} {mp.latest_period ? `· ${mp.latest_period}` : "· нет одобренных отчётов"}</div>
                    </div>
                ...
                {rm.mps.length === 0 && <div className="p-3 px-4 text-sm" style={{ color: "#6B7280" }}>В команде пока нет МП</div>}
              </div>
            )}
          </div>
        ))}
        {data.hierarchy.length === 0 && <div className="text-sm" style={{ color: "#6B7280" }}>Пока нет данных</div>}
      </div>
    </div>
  );
}
