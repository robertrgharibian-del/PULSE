    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-5 py-8">
      <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
        <div>
          ...
          <div className="text-sm" style={{ color: "#6B7280" }}>Мультипликатор × средний бонус команды МП</div>
        </div>
        <div className="flex gap-2">
          <select value={quarter} onChange={(e) => setQuarter(Number(e.target.value))} className="bg-transparent border rounded px-3 py-2 text-sm" style={{ borderColor: "#D3D8E4" }}>
            {[1, 2, 3, 4].map((q) => <option key={q} value={q} style={{ color: "#000" }}>Q{q}</option>)}
          </select>
          <input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} className="bg-transparent border rounded px-3 py-2 text-sm w-24" style={{ borderColor: "#D3D8E4" }} />
        </div>
      </div>

      {error && <div className="text-sm mb-4 px-3 py-2 rounded" style={{ background: "#DC262622", color: "#DC2626" }}>{error}</div>}
      {!data ? <div style={{ color: "#6B7280" }}>Загрузка…</div> : (
        <>
          <div className="rounded-2xl p-4 sm:p-5 mb-5" style={{ background: "#F7F8FC", border: "1px solid #E4E7F0" }}>
            ...
            <div className="hidden sm:block">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.team.map((t) => (
                    <tr key={t.mp_id} style={{ borderTop: "1px solid #E4E7F0" }}>
                      ...
                      <td className="text-right px-2" style={{ color: t.qualifies ? "#16A34A" : "#DC2626" }}>{t.qualifies ? "✓" : "✗"}</td>
                      <td className="text-right font-mono">{Math.round(t.bonus_uzs).toLocaleString()}</td>
                    </tr>
                  ))}
                  {data.team.length === 0 && <tr><td colSpan={5} className="py-4 text-center" style={{ color: "#6B7280" }}>В команде пока нет МП</td></tr>}
                </tbody>
              </table>
            </div>
            <div className="sm:hidden space-y-2">
              {data.team.map((t) => (
                <div key={t.mp_id} className="rounded-xl p-3" style={{ background: "#EEF1F8" }}>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-medium">{t.mp_name}</span>
                    <span style={{ color: t.qualifies ? "#16A34A" : "#DC2626" }}>{t.qualifies ? "✓" : "✗"}</span>
                  </div>
          ...
          <div className="rounded-xl p-4 flex items-center justify-between" style={{ background: "linear-gradient(90deg,#EEF1F8,#F7F8FC)" }}>
            ...
            <div className="font-mono text-xl font-bold" style={{ color: data.rm_bonus_uzs > 0 ? "#ED3237" : "#DC2626" }}>
