...
export const api = {
  ...
  restoreGroup: (id) => request(`/api/groups/${id}`, { method: "PATCH", body: { is_active: true } }),
  listBrands: (archived) => request(`/api/brands${archived ? "?archived=true" : ""}`),
  createBrand: (name, group_id) => request("/api/brands", { method: "POST", body: { name, group_id } }),
  updateBrand: (id, payload) => request(`/api/brands/${id}`, { method: "PUT", body: payload }),
  ...
  listRmTerritories: (rmId) => request(`/api/users/${rmId}/territories`),
  addRmTerritory: (rmId, territory) => request(`/api/users/${rmId}/territories`, { method: "POST", body: { territory } }),
  removeRmTerritory: (rmId, territoryId) => request(`/api/users/${rmId}/territories/${territoryId}`, { method: "DELETE" }),
